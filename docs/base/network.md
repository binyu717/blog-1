...
- 数据链路层
- 物理层
- 图例（[来自网站](https://blog.csdn.net/xw20084898/article/details/39438783)）：<img src="http://hi.csdn.net/attachment/201201/5/0_1325744597WM32.gif" alt="osi七层模型">

### 经典5层网络模型
- 客户端：
  - 应用层：http、ftp，基于tcp/ip定义http等协议传输方式;
  - 传输层：tcp/ip、udp，提供端到端的服务方式的定义;
  - 网络层：定义数据节点间传输数据的方式（路由器）;
  - 数据链路层：在物理层基础上，建立数据连接（网桥，交换机）;
  - 物理层：物理设备（网卡，网线，集线器，中继器，调制解调器）;
- 服务端：
  - 应用层;
  - 传输层;
  - 网络层;
